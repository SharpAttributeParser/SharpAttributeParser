...
public interface IAttributeMapper<in TRecord>
{
    ...
    /// <summary>Attempts to map the provided <see cref="IParameterSymbol"/> to an <see cref="IAttributeConstructorArgumentRecorder"/>.</summary>
    /// <param name="parameter">The <see cref="IParameterSymbol"/> representing the parameter.</param>
    /// <param name="dataRecord">The <typeparamref name="TRecord"/> to which the mapped <see cref="IAttributeConstructorArgumentRecorder"/> records arguments.</param>
    /// <returns>The mapped <see cref="IAttributeConstructorArgumentRecorder"/>, or <see langword="null"/> if the attempt was unsuccessful.</returns>
    /// <exception cref="ArgumentNullException"/>
    public abstract IAttributeConstructorArgumentRecorder? TryMapConstructorParameter(IParameterSymbol parameter, TRecord dataRecord);
