...
{
    private static DSyntacticArrayRecorder Target<T>(ISyntacticAdapterProvider adapters, Func<IReadOnlyList<T?>, CollectionLocation, bool> recorder) where T : struct => adapters.ForNullableElements(recorder);
