...
{
    private static DSyntacticArrayRecorder Target<T>(ISyntacticAdapterProvider adapters, Action<IReadOnlyList<T?>, CollectionLocation> recorder) where T : struct => adapters.ForNullableElements(recorder);
