...
public sealed class ForNullable_Action_Single_Struct
{
    private static DSyntacticSingleRecorder Target<T>(ISyntacticAdapterProvider adapters, Action<T?, Location> recorder) where T : struct => adapters.ForNullable(recorder);
