...
public sealed class TryParse_SimpleNamed
{
    ...
    private async Task ReturnsTrueAndRecords(string source, Func<AttributeSyntax, ExpectedArguments> expectedArgumentsDelegate)
    {
        ...
        Context.RecorderMock.Verify((recorder) => recorder.NamedArgument.TryRecordArgument(nameof(NamedAttribute.SimpleValue), expectedArguments.ValueSyntax!), Times.Exactly(expectedArguments.ValueSyntaxSetCount));
    }
    ...
    private sealed class ExpectedArguments
    {
        ...
        private bool ValueSyntaxSet { get; init; }

        private ExpressionSyntax? RawValueSyntax { get; init; }
    }
}
