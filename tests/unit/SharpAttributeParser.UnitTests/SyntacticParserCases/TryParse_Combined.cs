...
public sealed class TryParse_Combined
{
    ...
    private async Task ReturnsTrueAndRecords(string source, Func<AttributeSyntax, ExpectedArguments> expectedArgumentsDelegate)
    {
        ...
        Context.RecorderMock.Verify((recorder) => recorder.ConstructorArgument.TryRecordArgument(constructorParameters[0], expectedArguments.SimpleValueSyntax!), Times.Once);
        Context.RecorderMock.Verify((recorder) => recorder.ConstructorArgument.TryRecordArgument(constructorParameters[1], It.Is<ExpressionSyntax>(expectedArguments.ArrayValueSyntax!, SequenceEqualityComparer.Instance)), Times.Once);

        expectedArguments.ParamsValueSyntax.Switch
        (
            (syntax) => Context.RecorderMock.Verify((recorder) => recorder.ConstructorArgument.TryRecordArgument(constructorParameters[2], syntax), Times.Once),
            (elementSyntax) => Context.RecorderMock.Verify((recorder) => recorder.ConstructorArgument.TryRecordParamsArgument(constructorParameters[2], It.Is<IReadOnlyList<ExpressionSyntax>>(elementSyntax, SequenceEqualityComparer.Instance)), Times.Once)
        );

        Context.RecorderMock.Verify((recorder) => recorder.NamedArgument.TryRecordArgument(nameof(CombinedAttribute<object, object>.SimpleNamedValue), expectedArguments.SimpleNamedValueSyntax!), Times.Exactly(expectedArguments.SimpleNamedValueSyntaxSetCount));
        Context.RecorderMock.Verify((recorder) => recorder.NamedArgument.TryRecordArgument(nameof(CombinedAttribute<object, object>.ArrayNamedValue), expectedArguments.ArrayNamedValueSyntax!), Times.Exactly(expectedArguments.ArrayNamedValueSyntaxSetCount));
    }
