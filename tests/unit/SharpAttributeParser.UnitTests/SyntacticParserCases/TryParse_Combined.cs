...
public sealed class TryParse_Combined
{
    ...
    private async Task ReturnsTrueAndRecords(string source, Func<AttributeSyntax, ExpectedArguments> expectedArgumentsDelegate)
    {
        ...
        Context.RecorderMock.Verify((recorder) => recorder.Constructor.Normal.TryRecordArgument(constructorParameters[0], expectedArguments.SimpleValueSyntax!), Times.Once);
        Context.RecorderMock.Verify((recorder) => recorder.Constructor.Normal.TryRecordArgument(constructorParameters[1], It.Is<ExpressionSyntax>(expectedArguments.ArrayValueSyntax!, SequenceEqualityComparer.Instance)), Times.Once);

        expectedArguments.ParamsValueSyntax.Switch
        (
            (syntax) => Context.RecorderMock.Verify((recorder) => recorder.Constructor.Normal.TryRecordArgument(constructorParameters[2], syntax), Times.Once),
            (elementSyntax) => Context.RecorderMock.Verify((recorder) => recorder.Constructor.Params.TryRecordArgument(constructorParameters[2], It.Is<IReadOnlyList<ExpressionSyntax>>(elementSyntax, SequenceEqualityComparer.Instance)), Times.Once)
        );
