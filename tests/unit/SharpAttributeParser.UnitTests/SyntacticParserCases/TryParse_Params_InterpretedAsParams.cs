...
public sealed class TryParse_Params_InterpretedAsParams
{
    ...
    private async Task ReturnsTrueAndRecords(string source, Func<AttributeSyntax, ExpectedArguments> expectedArgumentsDelegate)
    {
        ...
        Context.RecorderMock.Verify((recorder) => recorder.ConstructorArgument.TryRecordParamsArgument(constructorParameters[0], It.Is<IReadOnlyList<ExpressionSyntax>>(expectedArguments.Value, SequenceEqualityComparer.Instance)), Times.Once);
    }
