...
{
    private static DSemanticArrayRecorder Target<T>(ISemanticAdapterProvider adapters, Action<IReadOnlyList<T?>?> recorder) where T : struct => adapters.ForNullable(recorder);
