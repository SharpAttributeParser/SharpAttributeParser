...
{
    private static DSemanticArrayRecorder Target<T>(ISemanticAdapterProvider adapters, Action<IReadOnlyList<T>> recorder) where T : notnull => adapters.For(recorder);
