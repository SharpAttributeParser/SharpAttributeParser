...
{
    private static void Target<TRecorder, TRecorderFactory>(ITypeMappingRepository<TRecorder, TRecorderFactory> repository, string parameterName, Func<TRecorderFactory, TRecorder> recorderDelegate) => repository.AddNamedMapping(parameterName, recorderDelegate);
    ...
    [Fact]
    public void AlreadyExistingsIndexedMappings_InvalidOperationException()
    {
        var context = RepositoryContext<object, object>.CreateWithOrdinalComparer();

        context.Repository.AddIndexedMapping(0, Mock.Of<object>());
