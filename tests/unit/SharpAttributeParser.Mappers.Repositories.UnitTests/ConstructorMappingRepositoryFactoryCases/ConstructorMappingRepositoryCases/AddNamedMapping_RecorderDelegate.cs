...
{
    private static void Target<TRecorder, TRecorderFactory>(IConstructorMappingRepository<TRecorder, TRecorderFactory> repository, string parameterName, Func<TRecorderFactory, TRecorder> recorderDelegate) => repository.AddNamedMapping(parameterName, recorderDelegate);
