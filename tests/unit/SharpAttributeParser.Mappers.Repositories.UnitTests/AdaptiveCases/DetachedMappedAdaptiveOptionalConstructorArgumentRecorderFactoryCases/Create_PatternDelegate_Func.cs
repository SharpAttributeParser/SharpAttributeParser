...
{
    private static IDetachedMappedAdaptiveConstructorArgumentRecorderProvider<TCombinedRecord, TSemanticRecord> Target<TCombinedRecord, TSemanticRecord, TArgument>(IDetachedMappedAdaptiveOptionalConstructorArgumentRecorderProviderFactory<TCombinedRecord, TSemanticRecord> factory, Func<IArgumentPatternFactory, IArgumentPattern<TArgument>> patternDelegate, Func<TCombinedRecord, TArgument, OneOf<None, ExpressionSyntax>, bool> combinedRecorder, Func<TSemanticRecord, TArgument, bool> semanticRecorder) => factory.Create(patternDelegate, combinedRecorder, semanticRecorder);
    ...
    public void ValidPatternAndRecorders_UsesCombinedAndSemanticFactories()
    {
        ...
        var combinedRecorderDelegate = Mock.Of<Func<object, object, OneOf<None, ExpressionSyntax>, bool>>();
        var semanticReecorderDelegate = Mock.Of<Func<object, object, bool>>();

        var context = FactoryContext<object, object>.Create();

        context.CombinedFactoryMock.Setup(static (factory) => factory.Create(It.IsAny<Func<IArgumentPatternFactory, IArgumentPattern<object>>>(), It.IsAny<Func<object, object, OneOf<None, ExpressionSyntax>, bool>>())).Returns(combinedRecorder);
