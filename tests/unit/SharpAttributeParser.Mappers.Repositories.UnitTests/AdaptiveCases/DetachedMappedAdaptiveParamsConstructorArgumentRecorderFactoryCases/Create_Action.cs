...
public sealed class Create_Action
{
    ...
    public void NullSemanticRecorder_ArgumentNullException()
    {
        ...
        var exception = Record.Exception(() => Target(context.Factory, Mock.Of<Action<object, object?, OneOf<ExpressionSyntax, IReadOnlyList<ExpressionSyntax>>>>(), null!));
    ...
    public void ValidRecorders_UsesCombinedAndSemanticFactories()
    {
        ...
        var combinedRecorderDelegate = Mock.Of<Action<object, object?, OneOf<ExpressionSyntax, IReadOnlyList<ExpressionSyntax>>>>();
        var semanticReecorderDelegate = Mock.Of<Action<object, object?>>();

        var context = FactoryContext<object, object>.Create();

        context.CombinedFactoryMock.Setup(static (factory) => factory.Create(It.IsAny<Action<object, object?, OneOf<ExpressionSyntax, IReadOnlyList<ExpressionSyntax>>>>())).Returns(combinedRecorder);
