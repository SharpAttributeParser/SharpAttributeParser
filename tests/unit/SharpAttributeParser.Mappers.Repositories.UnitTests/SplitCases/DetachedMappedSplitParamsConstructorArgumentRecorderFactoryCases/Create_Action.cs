...
{
    private static IDetachedMappedSplitConstructorArgumentRecorderProvider<TSemanticRecord, TSyntacticRecord> Target<TSemanticRecord, TSyntacticRecord>(IDetachedMappedSplitParamsConstructorArgumentRecorderProviderFactory<TSemanticRecord, TSyntacticRecord> factory, Action<TSemanticRecord, object?> semanticRecorder, Action<TSyntacticRecord, OneOf<ExpressionSyntax, IReadOnlyList<ExpressionSyntax>>> syntacticRecorder) => factory.Create(semanticRecorder, syntacticRecorder);
    ...
    public void NullSemanticRecorder_ArgumentNullException()
    {
        ...
        var exception = Record.Exception(() => Target(context.Factory, null!, Mock.Of<Action<object, OneOf<ExpressionSyntax, IReadOnlyList<ExpressionSyntax>>>>()));
