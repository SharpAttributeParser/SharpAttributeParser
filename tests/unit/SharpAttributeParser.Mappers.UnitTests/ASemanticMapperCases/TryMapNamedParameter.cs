...
{
    private static IMappedSemanticNamedArgumentRecorder? Target<TRecord>(ASemanticMapper<TRecord> mapper, string parameterName, TRecord dataRecord) => mapper.TryMapNamedParameter(parameterName, dataRecord);
    ...
    public void Mapped_ReturnsRecorderAndUsesDependencies()
    {
        ...
        var detachedRecorder = Mock.Of<IDetachedMappedSemanticNamedArgumentRecorder<object>>();

        var context = MapperContext<object>.Create();

        context.RecorderFactoryMock.Setup(static (factory) => factory.NamedParameter.Create(It.IsAny<object>(), It.IsAny<IDetachedMappedSemanticNamedArgumentRecorder<object>>())).Returns(expectedRecorder);
