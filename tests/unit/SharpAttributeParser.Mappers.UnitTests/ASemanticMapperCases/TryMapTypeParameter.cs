...
{
    private static IMappedSemanticTypeArgumentRecorder? Target<TRecord>(ASemanticMapper<TRecord> mapper, ITypeParameterSymbol parameter, TRecord dataRecord) => mapper.TryMapTypeParameter(parameter, dataRecord);
    ...
    public void MappedByName_ReturnsRecorderAndUsesDependencies()
    {
        ...
        var detachedRecorder = Mock.Of<IDetachedMappedSemanticTypeArgumentRecorder<object>>();

        var context = MapperContext<object>.Create();

        context.RecorderFactoryMock.Setup(static (factory) => factory.TypeParameter.Create(It.IsAny<object>(), It.IsAny<IDetachedMappedSemanticTypeArgumentRecorder<object>>())).Returns(expectedRecorder);
