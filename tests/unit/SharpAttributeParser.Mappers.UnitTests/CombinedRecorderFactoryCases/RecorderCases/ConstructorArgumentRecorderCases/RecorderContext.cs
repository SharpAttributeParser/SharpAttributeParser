...
internal sealed class RecorderContext<TRecord> where TRecord : class
{
    public static RecorderContext<TRecord> Create()
    {
        ...
        var recorder = ((ICombinedRecorderFactory)factory).Create(mapperMock.Object, dataRecordMock.Object).ConstructorArgument;

        return new(recorder, mapperMock, dataRecordMock, loggerFactoryMock);
    }

    public ICombinedConstructorArgumentRecorder Recorder { get; }
    ...
    private RecorderContext(ICombinedConstructorArgumentRecorder recorder, Mock<ICombinedMapper<TRecord>> mapperMock, Mock<TRecord> dataRecordMock, Mock<ICombinedArgumentRecorderLoggerFactory> loggerFactoryMock)
    {
        Recorder = recorder;
