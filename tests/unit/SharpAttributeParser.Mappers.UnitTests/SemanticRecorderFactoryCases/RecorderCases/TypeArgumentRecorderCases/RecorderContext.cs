...
internal sealed class RecorderContext<TRecord> where TRecord : class
{
    public static RecorderContext<TRecord> Create()
    {
        ...
        var recorder = ((ISemanticRecorderFactory)factory).Create(mapperMock.Object, dataRecordMock.Object).TypeArgument;

        return new(recorder, mapperMock, dataRecordMock, loggerFactoryMock);
    }

    public ISemanticTypeArgumentRecorder Recorder { get; }
    ...
    private RecorderContext(ISemanticTypeArgumentRecorder recorder, Mock<ISemanticMapper<TRecord>> mapperMock, Mock<TRecord> dataRecordMock, Mock<ISemanticArgumentRecorderLoggerFactory> loggerFactoryMock)
    {
        Recorder = recorder;
