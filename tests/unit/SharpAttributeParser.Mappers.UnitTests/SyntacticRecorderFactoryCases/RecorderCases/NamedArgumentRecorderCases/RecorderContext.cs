...
internal sealed class RecorderContext<TRecord> where TRecord : class
{
    public static RecorderContext<TRecord> Create()
    {
        ...
        var recorder = ((ISyntacticRecorderFactory)factory).Create(mapperMock.Object, dataRecordMock.Object).NamedArgument;

        return new(recorder, mapperMock, dataRecordMock, loggerFactoryMock);
    }

    public ISyntacticNamedArgumentRecorder Recorder { get; }
    ...
    private RecorderContext(ISyntacticNamedArgumentRecorder recorder, Mock<ISyntacticMapper<TRecord>> mapperMock, Mock<TRecord> dataRecordMock, Mock<ISyntacticArgumentRecorderLoggerFactory> loggerFactoryMock)
    {
        Recorder = recorder;
