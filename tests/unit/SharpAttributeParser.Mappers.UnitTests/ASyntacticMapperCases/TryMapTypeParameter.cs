...
{
    private static IMappedSyntacticTypeArgumentRecorder? Target<TRecord>(ASyntacticMapper<TRecord> mapper, ITypeParameterSymbol parameter, TRecord dataRecord) => mapper.TryMapTypeParameter(parameter, dataRecord);
    ...
    public void MappedByName_ReturnsRecorderAndUsesDependencies()
    {
        ...
        var detachedRecorder = Mock.Of<IDetachedMappedSyntacticTypeArgumentRecorder<object>>();

        var context = MapperContext<object>.Create();

        context.RecorderFactoryMock.Setup(static (factory) => factory.TypeParameter.Create(It.IsAny<object>(), It.IsAny<IDetachedMappedSyntacticTypeArgumentRecorder<object>>())).Returns(expectedRecorder);
